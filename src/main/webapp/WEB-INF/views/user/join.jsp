...
<script>
	...
	function fnCheckId() {
		$('#userId').on('keyup', function() {
			// 입력한 아이디
			let userId = $(this).val();
			// 정규식 (5~30자, 소문자+숫자+하이픈(-)+밑줄(_) 사용 가능, 첫 글자는 소문자+숫자 사용 가능)
			let regId = /^[a-z0-9][a-z0-9-_]{4,29}$/;
			// 정규식 검사
			verifyId = regId.test(userId);
			if (verifyId == false) {
				$('#msgId').text('5~30자, 소문자+숫자+하이픈(-)+밑줄(_) 사용 가능, 첫 글자는 소문자+숫자 사용 가능');
	...
	// 6. 년/월/일
	  function fnCreateDate(){
	    ...
	    $('#userBirthmonth').append(strMonth);
		  ...
		  // 일(월에 따른 연동)
	    $('#userBirthdate').append('<option value="">일</option>');
...
<body>
	...
	<div>
		...
		<form id="frmJoin" method="post" action="${contextPath}/user/join.do">
			...
			<div>
				<label for="userMobile">연락처*</label> 
				<input type="text" name="userMobile" id="userMobile" placeholder="하이픈을 제외하세요."> 
				<span id="msgMobile"></span>
			</div>
			<div>
				<label for="userBirthyear">생년월일*</label> 
				<select name="userBirthyear" id="userBirthyear"></select>
				<select name="userBirthmonth" id="userBirthmonth"></select>
				<select name="userBirthdate" id="userBirthdate"></select>
			</div>
			<div>
